...
describe("the board of the letter reader", () => {
  it("draws the twenty-five cells as buttons, pressed where the drawing is ink", () => {
    const html = renderLetters(new LetterReader([{ name: "A", pixels: glyphs.A! }, { name: "B", pixels: glyphs.B! }], 1), glyphs.T!);
  ...
  it("says, for each letter it was taught, how sure it is, and which it reads", () => {
    const reader = new LetterReader([{ name: "A", pixels: glyphs.A! }, { name: "B", pixels: glyphs.B! }], 1);
    reader.train(200);
  ...
  it("says so when it has not been taught anything yet", () => {
    expect(renderLetters(new LetterReader([{ name: "A", pixels: glyphs.A! }, { name: "B", pixels: glyphs.B! }], 1), glyphs.A!)).toContain("not been taught");
  });
});
