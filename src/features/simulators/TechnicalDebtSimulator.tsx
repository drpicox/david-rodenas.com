...
const TechnicalDebtSimulator = () => {
  ...
  const simulationData = useMemo(() => {
    ...
    for (let month = 0; month <= monthsToSimulate; month++) {
      ...
      if (cleanCumulative > debtCumulative && breakEvenPoint === null && month > 0) {
  ...
  const finalCleanFeatures = simulationData.data[simulationData.data.length - 1]?.cleanCumulative || 0;
  const finalDebtFeatures = simulationData.data[simulationData.data.length - 1]?.debtCumulative || 0;
  const finalDebt = simulationData.data[simulationData.data.length - 1]?.accumulatedDebt || 0;
  
  const productivityDiff = finalCleanFeatures > 0 ? ((finalCleanFeatures - finalDebtFeatures) / finalCleanFeatures * 100) : 0;
    ...
    <div className="w-full max-w-7xl mx-auto p-4 bg-gradient-to-br from-slate-50 to-blue-50 min-h-screen">
      ...
      <div className="bg-white rounded-lg shadow-md p-4 mb-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="space-y-1">
            <label className="block text-xs font-medium text-slate-700">
              Shortcuts: <span className="font-mono">{(shortcutFactor * 100).toFixed(0)}%</span>
            </label>
            <input
              type="range"
              min="0"
          ...
          <div className="space-y-1">
            <label className="block text-xs font-medium text-slate-700">
              Interest: <span className="font-mono">{(interestRate * 100).toFixed(0)}%</span>
            </label>
            <input
              type="range"
              min="0"
      ...
      <div className="flex gap-3 mb-4">
        <div className="bg-white rounded-lg shadow-sm p-3 border-l-4 border-green-500 flex-1">
          ...
          <div className="text-xl font-bold text-slate-800">{finalCleanFeatures}</div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-3 border-l-4 border-red-500 flex-1">
          ...
          <div className="text-xl font-bold text-slate-800">{finalDebtFeatures}</div>
          <div className="text-xs text-slate-500">features</div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-3 border-l-4 border-orange-500 flex-1">
          ...
          <div className="text-xl font-bold text-slate-800">
            {simulationData.breakEvenPoint ? `${simulationData.breakEvenPoint}mo` : 'Never'}
          </div>
          <div className="text-xs text-slate-500">crossover</div>
        </div>

        <div className={`bg-white rounded-lg shadow-sm p-3 border-l-4 ${isEven ? 'border-gray-500' : (isGain ? 'border-blue-500' : 'border-purple-500')} flex-1`}>
          <div className={`flex items-center gap-1 ${isEven ? 'text-gray-600' : (isGain ? 'text-blue-600' : 'text-purple-600')} mb-1`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
        ...
        <div className="bg-white rounded-lg shadow-md p-4">
          <h3 className="text-sm font-semibold text-slate-800 mb-3">Cumulative Features</h3>
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={simulationData.data} margin={{ top: 5, right: 5, left: 5, bottom: 35 }}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis 
                dataKey="month" 
                tick={{ fontSize: 10 }}
                label={{ value: 'Months', position: 'insideBottom', offset: -5, style: { fontSize: 10 } }}
              />
              <YAxis 
                tick={{ fontSize: 10 }}
                label={{ value: 'Features', angle: -90, position: 'insideLeft', style: { fontSize: 10 } }}
              />
              <Tooltip 
        ...
        <div className="bg-white rounded-lg shadow-md p-4">
          <h3 className="text-sm font-semibold text-slate-800 mb-3">Monthly Delivery Rate</h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={simulationData.data.slice(1)} margin={{ top: 5, right: 5, left: 5, bottom: 35 }}>
              <CartesianGrid strokeDasharray="3 3" />
              <XAxis 
                dataKey="month" 
                tick={{ fontSize: 10 }}
              ...
              <YAxis 
                tick={{ fontSize: 10 }}
                label={{ value: 'Features/Month', angle: -90, position: 'insideLeft', style: { fontSize: 10 } }}
              />
              <Tooltip 
                formatter={(value, name) => [
          ...
          <div className="space-y-2">
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600">
                  {interestRate === 0 ? 'No interest = no compound slowdown' : 
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600">
                  {(shortcutFactor * 100).toFixed(0)}% initial savings → {isEven ? 'break even' : `${productivityValue}% final ${productivityLabel.toLowerCase()}`}
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-600 font-mono">
                  {baseTime}d base × {(shortcutFactor*100).toFixed(0)}% cuts × {(interestRate*100).toFixed(0)}% interest
                </p>
              </div>
            </div>
          </div>
