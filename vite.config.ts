...
function site(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      // Anything happening under `content/` counts: a change, a new file, a
      // removed one, a new section. The watcher reports paths relative to the
      // root, so the test is for `content/` and not `/content/` — the leading
      // slash was the whole reason nothing had ever invalidated, and a new
      // page was served while the shell still said `cd: no such directory`.
