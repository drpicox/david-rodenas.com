...
It is like a database but for the frontend.
The Frontend reads it, and paints what is inside.
...
And the reducer, computes the new value of the sate, and
replaces its value by the new one with the action applied.
...
[DDD](/blog/2020/03/05/redux-and-domain-driven-development.html)
you can add perform chained actions as a consequence of an 
existing action:
...
after all subscribers has been dispatched,
it executes all after actions:
...
<Mermaid>{`sequenceDiagram
 ...
 Redux cannot handle asynchronous actions,
 but AfterActions can handle them. 
...
The LoadGame action activates the AfterAction.
The AfterAction dispatches the following ReplaceGame
...
<Mermaid>{`stateDiagram-v2
 ...
 And ofcourse, it also dispatches other
 actions like IncrementLoading or DecrementLoading.

### Handling Errors

Views cannot handle errors that happens in 
AfterActions. They shouldn't. If an error happens
in an AfterAction it should handle it and dispatch
the appropiate action. It is the state who stores
the problem and the View who shows the state.
...
In a previous section you have seen that actors
are the ones who dispatches actions:
...
<Mermaid>{`sequenceDiagram
 ...
 - External Actors:
   The users of the View, that clicking, typing, ... and interacting
   dispatches actions to the Store. 

- Internal Actors:
  They are AfterActions that once an action is completed, 
  it dispatches other actions to complete a Process.

### Process, Glue, and Decoupling

AfterActions are not only meant for asynchronous, they
are also to decouple packages of Redux.

You can decouple two packages through AfterActions. When
an existing action in an other package can trigger a new chain 
of changes, instead of modifying the original package,
use an AfterAction to trigger more changes.
...
> Never let an action dispatched by one Reduxe package
> to reduce inside another Redux package.
> Use an AfterAction instead.
...
The package C might want to know when B is updated,
and it could be updated because A, but also because B.
...
Subscribe to the store and get the current selector result 
with `useSelector`.
      ...
      {list.map((post) => (
        <div className={headerClassName}>
          <li key={post.id} onClick={() => go(post.id)} >
            <span role="button" style={{fontSize:"1.5rem"}}>{post.title}</span> »
          </li>
        </div>
      ))}
    </ul>
...
Ease the state manipulation with [object-path-immutable](https://www.npmjs.com/package/object-path-immutable).
