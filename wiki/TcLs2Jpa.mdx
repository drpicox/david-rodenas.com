...
JPA decouples Java implementation from
Database connection
...
Use annotations to describe how
to connect Objects to Database rows.
...
If you want to store in the database a new entity, add the
`@Entity` annotation in the class definition:
...
Use it with `@Id` when you want a new _random_ identifier for each
new Entity.
...
Use `@ManyToOne` to create one relationship from one entity
to another entity, when there are many of the current entity
referencing the same other entity.
...
Use `@OneToMany` to create one relationship from one entity
to another entity, when there the current entity has many
references to the other entity.
...
>> we have opted for the reverse relationship,
>> in order to decouple dependencies between packages.

>> We do not need this reverse relationship. 
>> See Repositories.

### @ManyToMany

Use this annotation when many entities connects to
other many entities.
...
Use this annotation when there is only one instance
of this type of entity in relationship with another.
...
There are many strategies to save entities
with inheritance.
...
Tables for each entity, but with all the fields without joind and may have nulls.
...
Embed other class attributes directly to your entity.
...
>> Note that the first Generic parameter is the Entity
>> the second is the primary key type.
>> The primary key type can be also the specified by an
...
- `findById(key: K): Optional<E>`: returns the entity corresponding to that key, if it exists
- `save(entity: E): E`: saves the current entity and returns it
- `delete(entity: E)`: deletes an entity from the database 
   (note that the instance still exists until you loose the reference)

### Implementing JpaRepository

Spring automatically implements JpaRepository.
All you have to do is to inject it inside a @Component 
and Spring automatically implements and injects it:
...
>> Note that it fails if some annotations are wrong.
>> Please, read errors carefully to discover which entity is wrong.

### Automatic Custom Queries

Spring implements automatically methods that are making queries on the 
database.
...
See naming rules here https://docs.spring.io/spring-data/jpa/docs/1.5.0.RELEASE/reference/html/jpa.repositories.html

### Manual queries

It is possible to write manual queries.
Do not use them on your assignment.


## Lifecycle

Entities have a lifecycle state graph about their relation with
the database. They can work without a database, or inside one.
...
Although JPA is vendor independent,
it does not protect you to fail if you use a name
