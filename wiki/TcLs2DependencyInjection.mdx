---
title: TcLs2 Dependency Injection
deck: true
---

## Testing Induced patterns

We have seen some patterns that testing induces us to use that all
work together and improves the composability of the system:
...
Spring defines as lower 's' singleton any class that
it is annotated by:
...
Spring automatically instances and injects singleton into
other singletons constructors.
...
Any singleton can request for an instance of any type,
if for that interface there is one concrete class, it injects
the corresponding concrete class.
...
In fact, instead of injecting one single instance,
it injects one instance for each singleton that
...
But there are interfaces in typescript.
...
Classes can act as interfaces in typescript and we can implement (instead of extend) them:
...
The `__test__` injector uses the main injector,
and adds all the necesary logic and mocks for the test.
